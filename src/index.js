...
const createInfoMarkup = (data) => {
	return data.map(
		({ name, capital, population, flags, languages }) =>
			'<h1><img src="${flags.png}" alt="${name.official}" width="40" height="40">${name.official}</h1><p>Capital: ${capital}</p><p>Population: ${population}</p><p>Languages: ${Object.values(languages)}</p>',
	);
};
